...
const LineBot = require("./src/lineBot.js");
...
(async () => {
   ...
   if (config.lineBot.enable) {
      ...
      //setup the bot
      console.log(`Initializing ${config.lineBot.type} Bot`);

      var lineBot = LineBot.init(config);
      ...
      function connectHost() {
         ...
         client
            ...
            .on("end", function () {
               endCounter += 1;
               console.log(
                  `${endCounter}: Connection to Host received: 'end'. `
               );
               reconnectToHost();
            });
      }
   ...
   if (config.slackBot.enable) {
      ...
      // Keep the process running so that the contianer doesn't constantly restart
      setInterval(() => console.log("Bot not enabled"), 1000 * 60 * 60);
   }
})();
